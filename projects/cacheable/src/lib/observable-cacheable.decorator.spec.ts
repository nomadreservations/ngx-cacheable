...
import { Cacheable, CacheBuster, globalCacheBusterNotifier } from '..';
...
const strategies: any[] = [null, DOMStorageStrategy];
strategies.forEach(s => {
  ...
  describe('CacheableDecorator', () => {
    ...
    it('should work correctly with a custom storage strategy', () => {
      ...
      jasmine.clock().mockDate();
      const asyncFreshData = _timedStreamAsyncAwait(service.getDateWithCustomStorageStrategyProvided('test'), 1000);
      ...
      const cachedResponse = _timedStreamAsyncAwait(service.getDateWithCustomStorageStrategyProvided('test'));
      ...
      const cachedResponse2 = _timedStreamAsyncAwait(service.getDateWithCustomStorageStrategyProvided('test'));
      // this call will renew the updateAtIndex call count since it's used to renew the cache
      ...
      const cachedResponse3 = _timedStreamAsyncAwait(service.getDateWithCustomStorageStrategyProvided('test'));
    ...
    it('use the maxCacheCount from the GlobalCacheConfig', () => {
       ...
       */
      const parameters = ['test1', 'test2', 'test3', 'test4', 'test5'];
      parameters.forEach(async param => _timedStreamAsyncAwait(service.getData(param), 1000));
      /**
       * data for all endpoints should be available through cache by now
       */
      expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
       ...
       * this should return a maximum of 5 different cached responses
       */
      const cachedResponseAll = _timedStreamAsyncAwait(forkJoin(parameters.map(param => service.getData(param))));
       ...
       */
      const cachedResponseAll2 = _timedStreamAsyncAwait(
        forkJoin(newParameters.map(param => service.getData(param))),
        1000
      );
      expect(cachedResponseAll2).toEqual([
        { payload: 'test2' },
