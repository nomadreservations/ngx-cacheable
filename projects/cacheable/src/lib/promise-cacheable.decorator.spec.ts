...
import { PCacheable, PCacheBuster, promiseGlobalCacheBusterNotifier } from '..';
import { Cat } from '../../test/cat';
import { IService } from '../../test/service.interface';
...
strategies.forEach(s => {
  ...
  describe('PCacheableDecorator', () => {
    ...
    class Service {
      ...
      getDataWithCustomCacheResolverAndHasher(parameter: number) {
        return this.mockServiceCall(parameter);
      }

      @PCacheable()
      getWithAComplexType(parameter: Cat) {
        return this.mockServiceCall(parameter);
      }
    ...
    it('return cached data for 5 unique requests, then should bail to data source', async () => {
      ...
      await Promise.all(parameters.map(param => service.getDataWithMaxCacheCount(param)));
      /**
       ...
       */
      const cachedResponseAll = await Promise.all(parameters.map(param => service.getDataWithMaxCacheCount(param)));
       ...
       */
      const cachedResponseAll2 = await Promise.all(newParameters.map(param => service.getDataWithMaxCacheCount(param)));
    ...
    it('return cached data for 5 unique requests all available for 7500ms', async done => {
      ...
      const cachedResponse2 = await Promise.all(
        parameters.map(param => service.getDataWithMaxCacheCountAndExpiration(param))
      );
      expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
    ...
    it('return cached data up until new parameters are passed WITH a custom resolver function', async () => {
      const asyncFreshData = await service.getDataWithCustomCacheResolver('test1');
      expect(asyncFreshData).toEqual({ payload: 'test1' });
      expect(mockServiceCallSpy).toHaveBeenCalled();

      const asyncFreshData2 = await service.getDataWithCustomCacheResolver('test2');
      expect(asyncFreshData2).toEqual({ payload: 'test2' });
      expect(mockServiceCallSpy).toHaveBeenCalledTimes(2);

      const cachedResponse = await service.getDataWithCustomCacheResolver('test3', {
    ...
    it('should clear all caches when the global cache buster is called', async () => {
      ...
      const asyncFreshData1 = await service.getData('test1');
      ...
      const asyncFreshData2 = await service.getData('test2');
    ...
    it('should not change undefined parameters to null', async () => {
      ...
      let mockServiceCallWithMultipleParametersSpy = spyOn(
        service,
        'mockServiceCallWithMultipleParameters'
      ).and.callThrough();
    ...
    it('use the maxCacheCount from the GlobalCacheConfig', async () => {
       ...
       */
      const parameters = ['test1', 'test2', 'test3', 'test4', 'test5'];
      await Promise.all(parameters.map(param => service.getData2(param)));
      /**
       * data for all endpoints should be available through cache by now
       */
      expect(mockServiceCallSpy).toHaveBeenCalledTimes(5);
       ...
       * this should return a maximum of 5 different cached responses
       */
      const cachedResponseAll = await Promise.all(parameters.map(param => service.getData2(param)));
       ...
       * this should return a maximum of 5 different cached responses, with the latest one in the end
       */
      const cachedResponseAll2 = await Promise.all(newParameters.map(param => service.getData2(param)));
